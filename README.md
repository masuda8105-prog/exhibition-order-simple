...
- Slack共有欄の「写真フォルダから選ぶ」「カメラで撮影」で別紙写真を追加できる（最大6枚、1枚20MB）。共有PDFの控えの後ろに1写真1ページの添付資料を付ける。A4縦・余白8mmに対して見出しと画像を合計270mm内へ配置し、写真全体を切り取らず縮小する。通常の2部印刷には添付しない。カメラ起動は端末・ブラウザ対応に依存し、PCなどではファイル選択になる。
