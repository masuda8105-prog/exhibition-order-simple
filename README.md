...
印刷ボタンは端末内でPDFファイルを生成します。作成後の「PDFを開く・印刷」からPDFビューアで印刷するか、「PDFを保存」「PDFを共有（Slackなど）」を使います。Webページ自体の印刷を使わないため、ブラウザのURL・ページタイトルをPDFに追加しません。会社・お客様控え各1部と、共有時は写真1枚につき1ページを作成します。控えの文字は画像化されるため、PDF内で文字検索はできません。生成PDFはメモリ内のみで、入力変更や注文終了時に破棄されます。
