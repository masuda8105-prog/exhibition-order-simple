...
- 「会社控えをPDF保存（日本語）」は会社控えだけをブラウザ印刷画面へ出す。Slack投稿は手動で、投稿完了を確認して「Slackに共有済み」をチェックする。自動投稿／Slack投稿結果の自動検証は行わない。
- 通常の「2部を印刷」は全注文で会社控え→お客様控えを出力し、間で改ページする。会社控えは常に日本語、お客様控えは国内なら日本語・海外なら英語。両方の上部・下部に用途を表示する。商品名・顧客入力値は原文。明細が多い場合は内容を切り捨てず各控えが複数ページになる。
- Slack共有欄の「写真フォルダから選ぶ」「カメラで撮影」で別紙写真を追加できる（最大6枚、1枚20MB）。会社控えPDFに1写真1ページの添付資料を付ける。通常の2部印刷には添付しない。カメラ起動は端末・ブラウザ対応に依存し、PCなどではファイル選択になる。
